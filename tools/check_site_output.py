...
1. The deployable files exist (index.html, 404.html, style.css, CNAME), and
   the CNAME file names weather-skills.org.
...
def _check(out_dir: Path) -> int:
    ...
    cname = out_dir / "CNAME"
    if cname.is_file() and cname.read_text(encoding="utf-8").strip() != "weather-skills.org":
        errors.append(f"CNAME must contain weather-skills.org, found {cname.read_text(encoding='utf-8')!r}")
